<aside style="background-color: #FFC107" class="main-sidebar sidebar-light-primary elevation-4">
    <!-- Brand Logo -->
    <a href="index.php" class="brand-link">
      <img src="resimler/logo/Mahallemden_logo.png" alt="Mahallenden İste" class="brand-image img-circle elevation-3"
           style="opacity: .8">
      <span class="brand-text font-weight-light">Mahallenden İste</span>
    </a>
 
    <!-- Sidebar -->
    <div class="sidebar">
      <!-- Sidebar user panel (optional) -->
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
         <!-- <img src="dist/img/user2-160x160.jpg" class="img-circle elevation-2" alt="User Image"> -->
        </div>
        <div class="info">
          <a href="index.php" class="d-block">Admin Paneli</a>
        </div>
      </div>

      <!-- Sidebar Menu -->
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->
                  <li class="nav-item">
            <a href="index" class="nav-link">
           <i class="fa fa-home"></i>
              <p>
                Anasayfa
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
                        <li class="nav-item">
            <a href="kategori" class="nav-link">
             <i class="fas fa-bars"></i>
              <p>
                Kategoriler
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
                        <li class="nav-item">
            <a href="hakkimizda" class="nav-link">
             <i class="fas fa-address-card"></i>
              <p>
                Hakkımızda
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
           <li class="nav-item">
            <a href="abone" class="nav-link">
             <i class="fas fa-address-card"></i>
              <p>
                Aboneler
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
                        <li class="nav-item">
            <a href="slider" class="nav-link">
              <i class=" fas fa-image"></i>
              <p>
                Slider
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
                                  <li class="nav-item">
            <a href="siparisler" class="nav-link">
              <i class=" fas fa-image"></i>
              <p>
                Siparişler
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
                        <li class="nav-item">
            <a href="uyeler" class="nav-link">
              <i class="nav-icon fas fa-users"></i>
              <p>
                Üyeler
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>


                        <li class="nav-item">
            <a href="yorumlar" class="nav-link">
              <i class="nav-icon fas fa-comments"></i>
              <p>
                Yorumlar
                <span class="right badge badge-secondary">Yeni</span>
              </p>
            </a>
          </li>
          <li class="nav-item has-treeview menu-open">
            <a href="#" class="nav-link active">
              <i class="nav-icon fas fa-cogs"></i>
              <p>
                Ayarlar
                <i class="right fas fa-angle-left"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="ayarlar" class="nav-link active">
               <i class="fab fa-chrome"></i>
                  <p>Site ayarları</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="iletisim" class="nav-link">
            <i class="far fa-address-card"></i>
                  <p>İletişim ayarları</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="sosyalmedya" class="nav-link">
                  <i class="fab fa-instagram"></i>
                  <p>Sosyal medya ayarları</p>
                </a>
              </li>
                 <li class="nav-item">
                <a href="kullanici" class="nav-link">
                  <i class="far fa-user nav-icon"></i>
                  <p>Kullanıcı bilgileri</p>
                </a>
              </li>

                  <li class="nav-item">
                <a href="cikis" class="nav-link">
<i class="fas fa-sign-out-alt"></i>
                  <p>Çıkış yap</p>
                </a>
              </li>
            </ul>
          </li>
       
          
          
          
          
          
    
          
          
        
        
          
        
    
     
          
     
      
        </ul>
      </nav>
      <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
  </aside>